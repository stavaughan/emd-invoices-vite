...
const controllerLogic = {
  getCollectionItem: async (Collection, req, res) => {
    ...
    try {
      const collectionItem = await collectionsLogic.findByID(
        res,
        Collection,
        reqID
      );
  ...
  updateCollectionItem: async (Collection, req, res) => {
    ...
    try {
      const updatedCollectionItem = await collectionsLogic.updateCollection(
        res,
        Collection,
        reqID,
        reqBody
  ...
  updateCollectionItems: async (Collection, req, res) => {
    ...
    const collectionItemsToUpdate = await Collection.find({
      _id: { $in: ids },
    });
    ...
    try {
      ...
      for (const id of ids) {
        ...
        const updatedCollectionItem = await Collection.findByIdAndUpdate(
          id,
          documentData,
          { new: true }
        );
