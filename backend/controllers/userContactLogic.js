...
const userContactLogic = {
  addEmailMarkApproved: async (res, email) => {
    try {
      const newEntry = await EmailList.create({
      ...
      res.status(401).send({
        message: `Error! failed to add email ${email} to approved email list`,
      });
    }
  },
  ...
  updateEmailMarkApproved: async (res, id, email) => {
    try {
      ...
      res.status(401).send({
        message: `Error! failed to update email, ${email}, status to approved on email list`,
      });
    }
  },
  ...
  addApprovedEmail: async (res, email) => {
    try {
      ...
      const emailListItem = await EmailList.findOne({
        email: email.toLowerCase(),
      });
      ...
      if (emailOnList) {
        if (emailListItem?.approved) {
          ...
          const updatedEmail = await userContactLogic.updateEmailMarkApproved(
            res,
            emailListItem?._id,
            email
          );
        ...
        const newEmail = await userContactLogic.addEmailMarkApproved(
          res,
          email
        );
        return newEmail;
      ...
      res.status(401).send({
        message: `Error! failed to add or update email, ${email}, to approved email list`,
      });
    }
  },
  ...
  userContact: async ({
    ...
    const existingID =
      existingUser && existingUser?._id ? existingUser._id : '';
