...
const { findByID, validateUserRole } = collectionsLogic;
...
const activateNewUser = async (res, firstName, lastName, email, userRole) => {
  ...
  try {
    if (user?._id) {
      ...
      res.status(401).send({
        message: 'Error! Failed to update or create new user account.',
      });
    }
  } catch (error) {
...
const usersLogic = {
  ...
  handleUserActivation: async (req, res) => {
    ...
    try {
      ...
      const userUpdate = await userContactLogic.updateUserPassword(
        res,
        userID,
        hashedPassword
      );
  ...
  handleDeleteUser: async (Collection, req, res) => {
    ...
    try {
      // handle delete user
      const deletedUser = await Collection.findOneAndRemove({
        _id: reqID,
      }).exec();
