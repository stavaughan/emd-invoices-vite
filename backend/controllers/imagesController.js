...
const getUserContacts = async (res, userID, contactID, avatarID) => {
  ...
  const updatedContact = await updateCollection(res, Contact, contactID, {
    avatarID,
  });
...
const imagesController = {
   ...
   * @access Private
   */
  getInvoiceImages: asyncHandler(
    async (req, res) =>
      await cloudinaryControllers.getCloudinaryData({
        res,
  ...
  uploadAvatarImage: asyncHandler(async (req, res) => {
    ...
    try {
      if (userID) {
        const uploadedImage =
          await cloudinaryControllers.uploadCloudinaryAvatar({
            res,
            dataUrl: data.url,
            pid: avatarID,
