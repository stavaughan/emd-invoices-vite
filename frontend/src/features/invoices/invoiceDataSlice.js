...
const getFilteredInvoices = (invoices) => {
	const invoicesCurrent = invoices.filter(invoice => currentYear === invoiceYear(invoice));
	const invoicesPrevious = invoices.filter(invoice => previousYear === invoiceYear(invoice));
	return invoicesCurrent?.length > 0 ? invoicesCurrent : invoicesPrevious;
}

const resetInvoicesState = (state) => {
	state.invoices = sortBy(state.invoices, 'dateCreated', 'desc')
	state.filteredInvoices = getFilteredInvoices(state.invoices)
};
...
export const invoiceDataSlice = createSlice({
	...
	reducers: {
		...
		filterByYearFacetValue: (state, action) => {
			const year = action.payload.year
			const facet = action.payload.facet
			const value = action.payload.value
			state.filteredInvoices = state.invoices.filter(invoice => year === invoiceYear(invoice))
			state.filteredInvoices = state.filteredInvoices.filter(invoice => invoice[facet] === value)
			setSelectedToFirst(state)
		},
		// Refactor to use serializable payload only
		filterByYearStatus: (state, action) => {
			const year = action.payload.year
			const statusFilter = action.payload.statusFilter
			state.filteredInvoices = state.invoices.filter(invoice => year === invoiceYear(invoice))
			state.filteredInvoices = state.filteredInvoices.filter(statusFilter)
			setSelectedToFirst(state)
		},
...
export const {
	...
	filterByYearStatus: filterInvoiceByYearStatus,
	filterByYearFacetValue: filterInvoiceByYearFacetValue
