...
export const InvoicesProvider = props => {
  ...
  const customerBusName = useCallback(customer => {
    if (!customer?.contactName?.surname && !customer?.contactName?.given_name)
      return '';
  ...
  const invoiceDates = useCallback(
    invoice => {
      ...
      const terms = paymentTerms
        ? payableTerms.find(_ => _._id === paymentTerms)
        : {};
  ...
  const invoiceNumber = useCallback(() => {
    ...
    const day = [1, 2, 3, 4, 5, 6, 7, 8, 9].includes(today)
      ? `0${today}`
      : `${today}`;
  ...
  const newInvoiceNumber = useCallback(
    (busPfx, i = 0) => {
  ...
  const getTotalPaid = useCallback(
    payments => {
      const paymentArray = payments?.length
  ...
  const invoiceTotals = useCallback(
    ({ taxRate, payments, sentStatus, servicesInvoiced }) => {
      ...
      const balance = payments?.length
        ? netTotal - getTotalPaid(payments)
        : netTotal;
      return {
  ...
  const detailsRowData = useCallback(
    invoice => {
  ...
  const displayStatus = useCallback(
    invoice => {
      ...
      return {
        ...
        correctPayment:
          status.invoiceSent &&
          condition.paymentMade &&
          (condition.currentYearInvoice || status.partiallyPaid)
      };
    },
    [payStatus]
  );
  ...
  const totalPayments = useCallback(payments => {
    return payments?.length
      ? payments.map(_ => _.amount_paid).reduce((a, b) => a + b, 0)
      : 0;
  }, []);
  ...
  const displayStamp = useCallback(invoice => {
    const payments = invoice?.payments?.length
      ? invoice.payments.map(_ => _.amount_paid)
      : [];
